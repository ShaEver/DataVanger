    ...
    public static IReadOnlyList<ModuleStatus> Create()
    {
        return new List<ModuleStatus>
        {
            ...
            new("quarantine-v2", "Secure Quarantine V2", ModuleOperatingState.Active,
                "Wired through one UI-owned composition root: authenticated encryption + HMAC + Windows DPAPI CurrentUser, expected-hash binding, structured audit, integrity-verified async restore, and ConfirmedMalware-only automatic action. Legacy V1 is never restored."),
            new("scheduler", "Scheduler", ModuleOperatingState.Active,
                "Deterministic tick-driven scheduler; reading status starts no loops."),
            new("realtime-protection", "Real-time protection", ModuleOperatingState.Passive,
                "Desktop UI monitor is user-mode and lives only for the UI process. It monitors and can report a known-malicious hash, but is not a resident/service blocking product."),
            new("deep-scan-recursive", "Recursive Deep Scan pipeline", ModuleOperatingState.Prepared,
                "The staged recursive Deep Scan pipeline exists but is not selected by the normal product scan path by default."),
            ...
            new("http-update-transport", "HTTP update transport", ModuleOperatingState.Prepared,
                "Signed-only fail-closed path: HTTPS URL, feed id, key id, supported algorithm and pinned PEM are required before transport construction. Missing config performs zero network/writes; unsigned fallback was removed. User appsettings PEM is development/operator trust, not a production trust root."),
            new("windows-service", "Windows service mode", ModuleOperatingState.Disabled,
                "Installation disabled by a fail-closed security gate: --install exits 7 before elevation or sc.exe. Admin-gated idempotent --uninstall remains for recovery. The explicit --service host is not installed by this build."),
            new("named-pipe-ipc", "Named-pipe IPC (local)", ModuleOperatingState.Prepared,
                "Local IPC implementation has payload allowlist/size validation, but the disabled service installation means no default connected host or active protection channel exists."),
            ...
            new("amsi-runtime", "AMSI resident runtime", ModuleOperatingState.Passive,
                "Default-off observe-only provider: explicit submissions can produce ScriptObserved telemetry when a service runtime is running. No OS registration, patching, blocking, confirmation, or automatic action."),
            new("memory-scanner", "Memory scanner", ModuleOperatingState.Unavailable,
                "The default composition uses NullMemoryReader, so no supported memory reader is active. Memory evidence is outside per-file EngineComposition and never confirms malware."),
            ...
            new("behavioral-engine", "Behavioral engine", ModuleOperatingState.Prepared,
                "Evidence-only correlation is composed only with an opted-in resident runtime; the default disabled service has no active event producers. It never confirms malware or performs remediation."),
            new("self-protection", "Self-protection", ModuleOperatingState.Prepared,
                "Tamper event types and in-memory sinks exist, but no watchdog, enforcement or remediation is composed in the default runtime."),
