...
public class SignedFeedUpdaterEndToEndTests
{
    ...
    [Fact]
    public void Runner_ReturnsNull_WhenSignedUpdatesNotConfigured()
    {
        // Default settings: signed updates disabled -> runner is inert and the caller uses the
        // legacy path. (Exercises the gating used by App/MainWindow without any network.)
        var settings = new AppSettings();
        Assert.False(SignedFeedUpdateRunner.IsConfigured(settings));
        Assert.Null(SignedFeedUpdateRunner.TryApply(settings, NewRoot(), NewRoot()));
    }
}
