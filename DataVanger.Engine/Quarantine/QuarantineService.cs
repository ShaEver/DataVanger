...
public sealed class QuarantineService : IQuarantineService, IQuarantineIndex
{
    ...
    private readonly IQuarantineFileOperations _fileOperations;
    private readonly Action<string>? _testOriginalRemover;
    ...
    public QuarantineService(
        ...
        IQuarantineFileOperations? fileOperations = null)
    {
        ...
        _fileOperations = fileOperations ?? new SystemQuarantineFileOperations();
        _testOriginalRemover = originalRemover;
    }
    ...
    public async Task<QuarantineResult> QuarantineAsync(QuarantineRequest request, CancellationToken cancellationToken = default)
    {
        ...
        if (Path.GetFileName(request.SourcePath).Contains(':'))
            return Fail(QuarantineStatus.ReparsePointRejected, "Alternate data streams are not valid quarantine sources.", request.Classification);

        try
        {
            if (_fileOperations.HasReparsePointInPath(request.SourcePath) ||
                (!string.IsNullOrWhiteSpace(_quarantineStoreRoot) && _fileOperations.HasReparsePointInPath(_quarantineStoreRoot)))
                return Fail(QuarantineStatus.ReparsePointRejected, "Source or quarantine-store path contains a reparse point.", request.Classification);
        }
        catch (UnauthorizedAccessException)
        {
            return Fail(QuarantineStatus.SourceAccessDenied, "Path ancestry could not be validated.", request.Classification);
        }

        long size;
        string sha256;
        byte[] plaintext;
        QuarantineSourceHandle? source = null;
        try
        {
            source = _fileOperations.OpenSource(request.SourcePath);
            size = source.Stream.Length;
            ...
            if (_options.MaxInMemoryPayloadBytes <= 0 || size > _options.MaxInMemoryPayloadBytes || size > int.MaxValue)
            {
                source.Dispose();
                return Fail(QuarantineStatus.SourceTooLarge, "Source exceeds the bounded-memory limit of the current AEAD format.", request.Classification);
            }

            plaintext = GC.AllocateUninitializedArray<byte>((int)size);
            await source.Stream.ReadExactlyAsync(plaintext, cancellationToken).ConfigureAwait(false);
            sha256 = _crypto.ComputeSha256Hex(plaintext);
            if (!string.IsNullOrWhiteSpace(request.ExpectedSha256) &&
                !string.Equals(sha256, request.ExpectedSha256.Trim(), StringComparison.OrdinalIgnoreCase))
            {
                source.Dispose();
                return Fail(QuarantineStatus.HashFailed,
                    "Source hash no longer matches the content that was classified.", request.Classification);
            }
        }
        ...
        QuarantineStatus? originalRemovalFailureStatus = null;
        ...
        if (mayDelete)
        {
            ...
            {
                if (_fileOperations.HasReparsePointInPath(request.SourcePath))
                    throw new QuarantineSourceConflictException("Source path became a reparse point.");
                if (!_fileOperations.PathReferencesIdentity(request.SourcePath, source!.Identity))
                    throw new QuarantineSourceConflictException("Source identity changed after quarantine storage.");
                if (_testOriginalRemover is not null)
                {
                    _testOriginalRemover(request.SourcePath); // test-only compatibility seam
                    record.OriginalRemoved = !File.Exists(request.SourcePath);
                }
                else
                {
                    _fileOperations.DeleteOpenedSource(source!, request.SourcePath);
                    record.OriginalRemoved = true; // the opened identity is delete-pending
                }
                if (!record.OriginalRemoved) originalDeleteFailed = true;
            }
            catch (QuarantineSourceConflictException)
            {
                originalDeleteFailed = true;
                record.OriginalRemoved = false;
                originalRemovalFailureStatus = QuarantineStatus.SourceIdentityConflict;
            }
            catch (UnauthorizedAccessException)
            {
                originalDeleteFailed = true;
                record.OriginalRemoved = false;
                originalRemovalFailureStatus = QuarantineStatus.SourceAccessDenied;
            }
            catch (Exception)
            {
                originalDeleteFailed = true;
                record.OriginalRemoved = false;
                originalRemovalFailureStatus = QuarantineStatus.OriginalDeleteFailed;
            }
        }
        ...
        if (originalDeleteFailed)
        {
            ...
            {
                Status = originalRemovalFailureStatus ?? QuarantineStatus.OriginalDeleteFailed,
                QuarantineId = id,
                Record = record,
                OriginalDeleteFailed = true,
    ...
    public async Task<QuarantineIntegrityResult> VerifyAsync(string quarantineId, CancellationToken cancellationToken = default)
    {
        ...
        if (_options.MaxInMemoryPayloadBytes <= 0 || record.OriginalSize > _options.MaxInMemoryPayloadBytes)
            return Integrity(QuarantineStatus.SourceTooLarge, quarantineId, true, false, QuarantineRecordState.Corrupt,
                "Payload exceeds the bounded-memory limit of the current AEAD format.");
    ...
    public async Task<QuarantineRestoreResult> RestoreAsync(QuarantineRestoreRequest request, CancellationToken cancellationToken = default)
    {
        ...
        if (_options.MaxInMemoryPayloadBytes <= 0 || record.OriginalSize > _options.MaxInMemoryPayloadBytes)
            return RestoreFail(QuarantineStatus.SourceTooLarge, "Payload exceeds the bounded-memory limit of the current AEAD format.", request.QuarantineId, record);
        ...
        try
        {
            if (_fileOperations.HasReparsePointInPath(destination))
            {
                Array.Clear(plaintext, 0, plaintext.Length);
                return RestoreFail(QuarantineStatus.ReparsePointRejected, "Restore path contains a reparse point.", request.QuarantineId, record);
            }
        }
        catch (UnauthorizedAccessException)
        {
            Array.Clear(plaintext, 0, plaintext.Length);
            return RestoreFail(QuarantineStatus.SourceAccessDenied, "Restore path ancestry could not be validated.", request.QuarantineId, record);
        }
        ...
        try
        {
            ...
            {
                await temp.WriteAsync(plaintext, cancellationToken).ConfigureAwait(false);
                await temp.FlushAsync(cancellationToken).ConfigureAwait(false);
                temp.Flush(flushToDisk: true);
            }
            if (_fileOperations.HasReparsePointInPath(destination))
                throw new QuarantineSourceConflictException("Restore path changed to a reparse point before commit.");
        ...
        catch (QuarantineSourceConflictException ex)
        {
            TryDeleteTemp(tempPath);
            Array.Clear(plaintext, 0, plaintext.Length);
            return RestoreFail(QuarantineStatus.RestoreRaceConflict, "Restore race refused: " + ex.Message, request.QuarantineId, record);
        }
        catch (IOException) when (!request.AllowOverwrite && File.Exists(destination))
        {
            TryDeleteTemp(tempPath);
            Array.Clear(plaintext, 0, plaintext.Length);
            return RestoreFail(QuarantineStatus.RestoreRaceConflict, "Restore target appeared before atomic commit; overwrite refused.", request.QuarantineId, record);
        }
        catch (Exception ex)
        {
            TryDeleteTemp(tempPath);
