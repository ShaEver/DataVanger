...
public sealed class SignedUpdateService : ISignedUpdateService
{
    ...
    public UpdateApplyResult CheckAndApply()
    {
        ...
        try
        {
            ...
            try
            {
                _contentSink.Stage(feedId, manifest.Sequence, staged);
    ...
    public UpdateApplyResult Rollback()
    {
        ...
        try
        {
            ...
            {
                // Content restore is best-effort; the authoritative state has
                // already been rolled back. Surface as a structured result.
                Publish(UpdateResultKind.RollbackCompleted, "Rollback (state only)",
                    "State rolled back; content restore reported: " + ex.Message, RuntimeEventSeverity.Medium);
            }
