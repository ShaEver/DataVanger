...
- Compose Secure Quarantine V2 only through `QuarantineCompositionRoot`; the UI
  owns its DPAPI CurrentUser key. Never reintroduce the V1 deterministic key,
  AES-CBC path, boolean restore wrapper, or trust in `quarantine_index.json`.
