...
the Settings window (`DataVanger/SettingsWindow.xaml(.cs)`). The UI exposes 28 of 29
properties; `TrustedPublishers` (the curated base trusted-publisher list) is
...
- **`EnableTrayProtection`** (default off), **`SignatureUpdateUrl`** (string).
...
- Implementation: `DataVanger.Engine/Quarantine/QuarantineService.cs` with
  ...
  `DpapiQuarantineKeyProtector.cs` (DPAPI key protection on Windows).
...
## 7. Update behavior — **verification Active; HTTP transport Stub**
...
- **`HttpUpdateTransport` is a STUB** — every method throws
  `NotSupportedException("HTTP transport is not implemented in this phase.")`. No
  network update transport is active. Updates are telemetry-only and never a verdict.
