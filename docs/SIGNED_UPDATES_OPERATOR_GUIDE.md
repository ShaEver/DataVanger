...
- **Aplicação isolada.** O conteúdo verificado é projetado em arquivos de feed dedicados
  (dentro da versão imutável ativa do feed) — **nunca** sobrescreve as
...
|---|---|---|
| `HashBlacklist` | lista de SHA-256 maliciosos (um por linha; `#`/`;` = comentário) | `.signed-feed/<feed>/versions/<version>/known_malicious_sha256.feed.txt` |
| `HashAllowlist` | lista de SHA-256 conhecidos-seguros | `.signed-feed/<feed>/versions/<version>/known_safe_sha256.feed.txt` |
| `YaraRules` | regras `.yar` (texto) | `.signed-feed/<feed>/versions/<version>/yara_rules/feed__<id>.yar` |
...
O produto ainda não possui uma chave vendor embutida nem um trust store administrativo.
Portanto, configurar PEM em `%UserProfile%\DataVanger\appsettings.json` é permitido somente
como modo **development/operator**; não constitui raiz de confiança de produção. Chaves:
...
- Antes de qualquer rede, o cliente exige: toggle ativo, URL HTTPS absoluta, `FeedId`,
  `KeyId`, algoritmo suportado e PEM pinado.
- Configuração ausente/incompleta retorna `Disabled`/`NotConfigured`, sem rede e sem escrita.
- Não existe fallback não assinado. A propriedade legada `SignatureUpdateUrl` é ignorada e
  apagada pela migração de configuração; ela nunca é convertida em confiança implícita.

Quando uma atualização é aplicada, ela é gravada como versão imutável em
`Signatures\.signed-feed\<feed>\versions\<sequence>-<canonicalHash>`. O scanner lê
somente a versão indicada pelo `active.json` atômico e revalida todos os hashes do
`version.json`; nunca combina arquivos de duas versões. Baseline e listas do usuário
permanecem fora dessa árvore.
...
- **Rollback:** Active e LKG contêm versões completas e sobrevivem a processo novo/reboot.
  O journal registra `Staging`, `Verified`, `Activating`, `Active`, `RollbackPending` e
  `RolledBack`. `RollbackCompleted` só é retornado depois de conteúdo e state store
  apontarem para a mesma versão verificada.
- **Rotação de chave em laboratório/operator:** publique com novo `KeyId` e atualize
  `SignedUpdateKeyId` + `SignedUpdatePublicKeyPem`. Produção exige futuramente chave vendor
  embutida ou store administrativo autenticado.
...
- Leitura no scanner: `SignedFeedProjectionDiscovery.cs`, `SignatureDatabase.cs` e
  `LightweightYaraDatabase.cs` (uma versão ativa completa por feed)
