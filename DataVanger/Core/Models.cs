...
public class ScanOptions
{
    ...
    /// <summary>
    /// Beta 10 — opt-in clean-file result cache. When true, a file that was
    /// determined <b>known-safe</b> on a previous scan and is byte-identical
    /// (same path + last-write + length) <b>and</b> whose signature fingerprint is
    /// unchanged is skipped without re-analysis, counting as
    /// <see cref="ScanMetrics.SkippedCacheClean"/>.
    ///
    /// Safety: default OFF. The Deep profile always ignores this cache (full
    /// re-analysis), which is how Full (broad, cache-aware) and Deep (expensive,
    /// exhaustive) are separated. A clean-skip never bypasses a known-malicious
    /// check: any change to the blacklist/whitelist/YARA signatures changes the
    /// fingerprint and forces a full rescan.
    /// </summary>
    public bool        UseScanResultCache { get; set; } = false;
}
...
public class ScanMetrics
{
    ...
    public int      TrustCacheHits         { get; set; }
