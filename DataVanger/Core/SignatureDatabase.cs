...
public sealed class SignatureDatabase
{
    ...
    public static SignatureDatabase Load(string signatureRoot, string? legacyBlacklistPath = null, string? legacyWhitelistPath = null)
    {
        ...
        // Load each feed from one immutable, completely verified active version.
        foreach (string activeRoot in SignedFeedProjectionDiscovery.GetVerifiedActiveVersionRoots(signatureRoot))
        {
            LoadFile(Path.Combine(activeRoot, "known_malicious_sha256.feed.txt"), db.KnownMalicious);
            LoadFile(Path.Combine(activeRoot, "known_safe_sha256.feed.txt"), db.KnownSafe);
        }
