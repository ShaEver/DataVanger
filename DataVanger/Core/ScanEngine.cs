...
///   - persists reports, hash cache and local reputation.
///
...
public class ScanEngine : IScanEngine
{
    ...
    public string QIndexPath     => Path.Combine(QuarantineRoot, "quarantine_index.json");
    ...
    // consumed by the later phases. Detection-phase-scoped objects (QuarantineManager,
    // quarantineLock) are intentionally NOT held here.
    ...
    private ScanState LoadConfiguration(Action<string> log)
    {
        ...
        var trustFingerprint = ComputeTrustFingerprint();

        return new ScanState(_settings, _signatures, yaraDb, deps, pipeline, hashService, reputation, reputationEngine, prevHashes, logger, metrics, new object(), new ScanStageProfiler(), trustCache, trustFingerprint);
    }
        ...
        foreach (var target in targets)
        {
            ...
            foreach (var file in deps.FileSystem.EnumerateFiles(target, ct, _ => Inc(m => m.AccessDenied++)))
            {
                ...
                try
                {
                    ...
                    if (!deep && PathTaxonomy.IsTrustedPath(fullLow)) { Inc(m => m.SkippedTrustedPath++); continue; }
    ...
    private async Task<ScanFinding?> AnalyzeSingleFileAsync(
        ...
        bool useResultCache, string signatureFingerprint, DetectionModuleSet modulesToRun)
    {
        ...
        // ---- Beta 10. Clean-file result cache (opt-in; Deep always re-analyzes).
        // Skips re-analysis only for an unchanged, previously known-safe file under
        // an unchanged signature fingerprint — never bypasses a known-malicious check.
        if (useResultCache && state.HashService.TryCleanSkip(file, signatureFingerprint))
        {
            Inc(m => m.SkippedCacheClean++);
            return null;
        }
        ...
        if (!isKnownMalware && !hasConfirmedSignature && knownSafeHash)
        {
            ...
            hashService.TouchScore(file.FullName, 0, knownSafe: true, signatureFingerprint);
        ...
        if (!isKnownMalware && !hasConfirmedSignature && preScore < minPreScore)
        {
            ...
            hashService.TouchScore(file.FullName, preScore, knownSafe: false, signatureFingerprint);
        ...
        if (isKnownMalware || hasConfirmedSignature || preScore >= sigThreshold)
        {
            ...
            if (state.TrustCache.TryGet(file, state.TrustFingerprint, out var cachedSig))
            {
                sig = cachedSig;
                Inc(m => m.TrustCacheHits++);
            }
            else
            {
                sig = GetSigInfo(file.FullName, allowCatalog);
                state.TrustCache.Store(file, state.TrustFingerprint, sig);
            }
        ...
        {
            hashService.TouchScore(file.FullName, score, knownSafe: false, signatureFingerprint);
        ...
        ScanState state, ScanOptions options, FileInfo file, ScanTarget target)
    {
        ...
        if (!(options.MaxHashSizeMB > 0 && file.Length > options.MaxHashSizeMB * 1_048_576L))
        {
            ...
            if (hash != null)
            {
                ...
                if (cacheHit) state.Inc(m => m.HashCacheHits++); else state.Inc(m => m.HashComputed++);
        ...
        Action<ScanProgressInfo>? onProgressInfo, CancellationToken ct)
    {
        ...
        // Beta 10 — clean-file cache posture. Full/Standard/Quick may reuse a clean
        // verdict for unchanged files; Deep always performs a full re-analysis
        // (that is how the optimized Full and the exhaustive Deep are separated).
        // The fingerprint binds a cached clean verdict to the exact signature state,
        // so any blacklist/whitelist/YARA change forces a full rescan.
        bool useResultCache = options.UseScanResultCache && options.Profile != ScanProfile.Deep;
        string signatureFingerprint = ComputeSignatureFingerprint(options.Profile, minPreScore, reportThreshold);
        if (useResultCache) log("[Scan] Cache de resultado limpo ativo (arquivos inalterados e seguros são reutilizados).");

        // --- 6. Run pipeline in parallel ------------------------------------
        var swScan = Stopwatch.StartNew();
        var qm = new QuarantineManager(QuarantineRoot, QIndexPath);
        using var quarantineLock = new SemaphoreSlim(1, 1);
        int processed = 0;
        ...
        }, async (file, token) =>
        {
            ...
            try
            {
                var finding = await AnalyzeSingleFileAsync(file, token, state, context, options, minPreScore, sigThreshold, persThreshold, reportThreshold, useResultCache, signatureFingerprint, modulesToRun).ConfigureAwait(false);
                ...
                {
                    string? qid;
                    await quarantineLock.WaitAsync(token).ConfigureAwait(false);
                    try
                    {
                        qid = qm.Quarantine(finding);
                    }
                    finally
                    {
                        quarantineLock.Release();
                    }
                    if (qid != null)
                    {
                ...
                Inc(m => m.Findings++);
                hashService.TouchScore(file.FullName, finding.Score, knownSafe: false, signatureFingerprint);
            }
    ...
    // Beta 10 — fingerprint of the active signature/rule state plus the gating
    // thresholds. The clean-file cache reuses a stored verdict only while this
    // fingerprint is unchanged, so any edit to the blacklist, whitelist, signature
    // folder or YARA rules forces a full rescan (no known-malicious bypass).
    // Cheap: a handful of file stats, computed once per scan.
    private string ComputeSignatureFingerprint(ScanProfile profile, int minPreScore, int reportThreshold)
    {
        var sb = new StringBuilder();
        sb.Append("v1|").Append(profile).Append('|').Append(minPreScore).Append('|').Append(reportThreshold);
        AppendPathStamp(sb, BlacklistPath);
        AppendPathStamp(sb, WhitelistPath);
        AppendNewestStamp(sb, SignatureRoot);
        return sb.ToString();
    }

    // Fingerprint of the trust-verification environment for the signature-trust cache. The OS
    // catalog store changes on Windows updates (catalogs added/removed), which can change a file's
    // catalog-signed state WITHOUT the file itself changing; stamping the CatRoot folder's
    // last-write invalidates cached trust results when that happens. Cheap (one directory stat).
    private static string ComputeTrustFingerprint()
    {
        try
        {
            ...
            {
                var catRoot = Path.Combine(windir, "System32", "CatRoot");
                if (Directory.Exists(catRoot)) stamp = Directory.GetLastWriteTimeUtc(catRoot).Ticks;
            }
            return $"trust-v1|{stamp}";
        }
        catch (Exception)
        {
            return "trust-v1|0";
    ...
    private static void AppendPathStamp(StringBuilder sb, string path)
    {
        try
        {
            if (File.Exists(path))
            {
                ...
                sb.Append('|').Append(fi.LastWriteTimeUtc.Ticks).Append(':').Append(fi.Length);
            }
            else sb.Append("|-");
        }
        catch (Exception)
        {
            // Unreadable signature file - fold an explicit marker into the fingerprint
            // so an unstable signature source never silently produces a stable key.
            sb.Append("|?");
        }
    }

    private static void AppendNewestStamp(StringBuilder sb, string dir)
    {
        try
        {
            if (!Directory.Exists(dir)) { sb.Append("|-"); return; }
            ...
            foreach (var f in Directory.EnumerateFiles(dir, "*", SearchOption.AllDirectories))
            {
                ...
                {
                    // Skip a single unreadable signature file; the remaining stamps still
                    // change the fingerprint whenever the rule set changes.
                }
            }
            sb.Append('|').Append(newest).Append(':').Append(total).Append(':').Append(count);
        }
        catch (Exception)
        {
            sb.Append("|?");
    ...
    private void WriteTxt(List<ScanFinding> findings, ScanMetrics m, ScanOptions opts)
    {
        try
        {
            ...
            sb.AppendLine($"SkippedTrustedPath     : {m.SkippedTrustedPath}");
            sb.AppendLine($"SkippedKnownSafe       : {m.SkippedKnownSafe}");
            sb.AppendLine($"SkippedCacheClean      : {m.SkippedCacheClean}");
            sb.AppendLine($"SkippedLowScore        : {m.SkippedLowScore}");
            sb.AppendLine($"HashComputed           : {m.HashComputed}");
            sb.AppendLine($"HashCacheHits          : {m.HashCacheHits}");
            ...
            sb.AppendLine($"TrustCacheHits         : {m.TrustCacheHits}");
