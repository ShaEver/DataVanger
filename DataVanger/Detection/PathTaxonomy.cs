...
/// reuse the same trust/risk vocabulary without re-deriving it. New trusted
/// vendor paths should be added here in one place.
///
...
public static class PathTaxonomy
{
    ...
    public static bool IsTrustedPath(string fullLower) =>
    ...
    public static bool IsProgramVendorPath(string fullLower) =>
        HasPrefix(fullLower, _tpProgramFiles)
        || HasPrefix(fullLower, _tpProgramFilesX86)
        || HasPrefix(fullLower, _tpCommon)
        || IsTrustedPath(fullLower);
    ...
    public static bool IsKnownBenignScriptContainer(string fullLower) =>
        fullLower.Contains("\\node_modules\\")
        || fullLower.Contains("\\.vscode\\extensions\\")
        || fullLower.Contains("\\.antigravity\\extensions\\")
        || fullLower.Contains("\\openai\\codex\\bin\\")
        || fullLower.Contains("\\microsoft vs code\\")
        || fullLower.Contains("\\programs\\microsoft vs code\\")
        || fullLower.Contains("\\resources\\app\\extensions\\")
        || fullLower.Contains("\\resources\\app\\out\\")
        || fullLower.Contains("\\microsoft\\office\\solutionpackages\\")
