...
public static class HeuristicAnalyzer
{
    ...
    public static AnalysisResult Analyze(ScanTarget target, ScanContext context)
    {
        ...
        if (context.RunningProcessPaths.Contains(full) && userWritable && !protectedWindows)
    ...
    /// <summary>
    /// Applies bounded noise relief only for a canonical protected-Windows path.
    /// When the file lives in that protected path AND no strong
    ...
    /// </summary>
    public static int ApplyProtectedWindowsWeakSignalAttenuation(ScanTarget target, AnalysisResult heuristicResult)
    {
        ...
        if (protectedWindows && !strongBehavior)
        {
            heuristicResult.Evidence.Clear();
            heuristicResult.Evidence.Add(new Evidence
            {
                Category = "Heuristic",
                Description = "Caminho protegido do Windows; heurísticas isoladas atenuadas para evitar falso positivo",
                ScoreDelta = 0,
