                ...
                switch (ReviewActionLabels.FromLabel(row.Action))
                {
                    case ReviewAction.Quarantine:
                        var request = QuarantineRequestFactory.Create(
                            row.Finding,
                            QuarantineRequestOrigin.ManualUserApproved,
                            "ReviewFixWindow");
                        var result = await _engine.Quarantine.Service
                            .QuarantineAsync(request, _lifetimeCts.Token);
                        if (result.Status == QuarantineStatus.Success &&
                            result.Record?.RecordState == QuarantineRecordState.Verified)
                        {
