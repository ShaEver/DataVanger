...
public sealed class LibyaraEngine : IYaraEngine, IDisposable
{
    ...
    private LibyaraEngine(string yaraRulesDirectory, Action<string>? log)
    {
        ...
        try
        {
            ...
            int added = 0;
            var ruleRoots = new List<string> { yaraRulesDirectory };
            string? signatureRoot = Directory.GetParent(yaraRulesDirectory)?.FullName;
            ...
            foreach (var path in ruleRoots.SelectMany(root => Directory.EnumerateFiles(root, "*.*", SearchOption.AllDirectories)))
            {
