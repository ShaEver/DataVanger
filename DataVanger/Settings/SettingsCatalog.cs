...
public static class SettingsCatalog
{
    ...
    private static IReadOnlyList<ToggleSetting> BuildToggles() => new List<ToggleSetting>
    {
        ...
        Toggle("EnableHttpSignedUpdates", "Atualizações assinadas via HTTP (opt-in)",
            "Habilita HTTPS assinado em modo development/operator. Sem trust root vendor/admin, não é configuração de produção.",
    ...
    private static IReadOnlyList<SettingMetadata> BuildMetadataOnly() => new List<SettingMetadata>
    {
        ...
        // Signed-feed configuration (consumed by SignedFeedUpdateRunner when the HTTP signed-update
        // toggle is enabled). Editors live in the legacy advanced settings window / appsettings.json;
        // see docs/SIGNED_UPDATES_OPERATOR_GUIDE.md.
        Meta("SignedUpdateFeedUrl", "URL do feed assinado (HTTPS)",
            "Origem HTTPS do manifesto assinado. Usada quando 'Atualizações assinadas via HTTP' está ativo.",
            SettingGroup.Updates, SettingKind.Text, SettingVisibility.Developer, SettingRisk.Caution, "url"),
        Meta("SignedUpdatePublicKeyPem", "Chave pública do operador (PEM)",
            "Chave pinada em modo development/operator. appsettings do usuário não é raiz de confiança de produção.",
            SettingGroup.Updates, SettingKind.Text, SettingVisibility.Developer, SettingRisk.Caution, "pem"),
        Meta("SignedUpdateKeyId", "ID da chave fixada",
            "Identificador (KeyId) usado para casar a assinatura do manifesto com a chave pública.",
            SettingGroup.Updates, SettingKind.Text, SettingVisibility.Developer, SettingRisk.Caution, "id"),
        Meta("SignedUpdateAlgorithm", "Algoritmo de assinatura",
            "RSA-PSS-SHA256 ou ECDsa-P256-SHA256.",
            SettingGroup.Updates, SettingKind.Text, SettingVisibility.Developer, SettingRisk.Caution, "algorithm"),
        Meta("SignedUpdateFeedId", "ID do feed",
            "Identificador lógico do feed; o controle anti-downgrade é por feed.",
            SettingGroup.Updates, SettingKind.Text, SettingVisibility.Developer, SettingRisk.Caution, "id"),
