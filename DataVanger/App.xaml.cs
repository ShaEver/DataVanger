...
public partial class App : WpfApplication
{
    ...
    protected override async void OnStartup(StartupEventArgs e)
    {
        ...
        try
        {
            ...
            {
                int exitCode = await RunSignatureUpdateAsync();
                Shutdown(exitCode);
                return;
            }
    ...
    private static async Task<int> RunSignatureUpdateAsync()
    {
        var engine = new ScanEngine();
        var settings = AppSettings.Load(engine.SettingsPath);

        var result = await Task.Run(() => SignedFeedUpdateRunner.Run(
            settings, engine.SignatureRoot, Path.Combine(engine.MgRoot, "UpdateState")));
        var writer = result.ExitCode == SignedFeedRunResult.ExitSuccess ? Console.Out : Console.Error;
        writer.WriteLine($"DataVanger signed update [{result.ExitCode}]: {result.Message}");
        if (result.ExitCode == SignedFeedRunResult.ExitRejectedOrFailed)
            WriteStartupCrashLog(new InvalidOperationException("Atualização assinada recusada: " + result.Message));
