...
public static class AmsiProviderRegistration
{
    ...
    public static int Unregister(TextWriter output, TextWriter error)
    {
        ...
        return UnregisterWindows(output, error);
    }
    ...
    [SupportedOSPlatform("windows")]
    private static int UnregisterWindows(TextWriter output, TextWriter error)
    {
        if (!IsElevated())
        {
            error.WriteLine(
        ...
        int lastFailure = 0;
        foreach (var command in BuildUnregisterPlan(ProviderClsid))
        {
            // Best-effort: a missing key is not a hard error (idempotent removal),
            // but a genuine failure is surfaced.
            int code = RunCommand(command, output, error);
            if (code != 0) lastFailure = code;
        }

        if (lastFailure != 0)
        {
            error.WriteLine(
                $"AMSI provider removal reported exit code {lastFailure} " +
                "(this is expected if it was not registered).");
        }

        output.WriteLine($"AMSI provider '{DisplayName}' unregistered (CLSID {ProviderClsid}).");
        return ExitOk;
    }
