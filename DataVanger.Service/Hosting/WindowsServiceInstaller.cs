...
public enum RecoveryOperationResult
{
    Success,
    NotFound,
    Failed,
...
/// <summary>Injectable recovery-only SCM boundary used by uninstall tests.</summary>
public interface IServiceRecoveryOperations
{
    bool IsSupportedPlatform { get; }
    bool IsElevated { get; }
    RecoveryOperationResult StopService(TextWriter output, TextWriter error);
    RecoveryOperationResult DeleteService(TextWriter output, TextWriter error);
}
...
public static class WindowsServiceInstaller
{
    ...
    public static int Install(TextWriter output, TextWriter error, string? configPath = null)
    {
        ...
        // P0 safety gate: this is deliberately before platform/elevation/path
        // checks so no installation preparation or external process can occur.
        if (!PrivilegedActivationGate.SecureInstallerAvailable)
        {
            error.WriteLine(PrivilegedActivationGate.ServiceInstallMessage);
    ...
    public static int Uninstall(TextWriter output, TextWriter error)
        => Uninstall(output, error, new WindowsServiceRecoveryOperations());

    /// <summary>
    /// Testable recovery path. A missing service is success, making repeated
    /// cleanup safe without weakening the administrator gate.
    /// </summary>
    public static int Uninstall(
        TextWriter output,
        TextWriter error,
        IServiceRecoveryOperations operations)
    {
        ArgumentNullException.ThrowIfNull(output);
        ArgumentNullException.ThrowIfNull(error);
        ...
        _ = operations.StopService(output, error); // best-effort and idempotent
        var delete = operations.DeleteService(output, error);
        if (delete == RecoveryOperationResult.Failed)
        {
            error.WriteLine("Service removal failed.");
            return ExitCommandFailed;
        }

        output.WriteLine(delete == RecoveryOperationResult.NotFound
            ? $"Service '{ServiceName}' was already absent."
    ...
    private sealed class WindowsServiceRecoveryOperations : IServiceRecoveryOperations
    {
        ...
        public bool IsElevated
        {
            get
            {
                ...
                return WindowsServiceInstaller.IsElevated();
            }
        }

        public RecoveryOperationResult StopService(TextWriter output, TextWriter error)
            => Run(BuildUninstallPlan()[0], output, error);

        public RecoveryOperationResult DeleteService(TextWriter output, TextWriter error)
            => Run(BuildUninstallPlan()[1], output, error);

        private static RecoveryOperationResult Run(
            ServiceControlCommand command,
            TextWriter output,
            TextWriter error)
        {
            if (!OperatingSystem.IsWindows()) return RecoveryOperationResult.Failed;
            int code = RunCommand(command, output, error);
            // ERROR_SERVICE_DOES_NOT_EXIST. This is the expected result for
            // repeated recovery and must not turn cleanup into a failure.
            if (code == 1060) return RecoveryOperationResult.NotFound;
            return code == 0 ? RecoveryOperationResult.Success : RecoveryOperationResult.Failed;
        }
    }
}
