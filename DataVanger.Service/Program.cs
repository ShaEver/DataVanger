...
///   --install         — unavailable until a secure installer is implemented;
///                       fails closed without elevation or system changes.
///   --uninstall       — stop (best-effort) and remove the Windows service
///                       (explicit, admin-gated, Windows-only).
///   --register-amsi-provider   — unavailable until a secure installer and
///                       signed native build exist; fails closed.
...
internal static class Program
{
    ...
    private static void PrintUsage()
    {
        ...
        Console.WriteLine("  --install           Unavailable until a secure installer is implemented (exit 7; no changes).");
        Console.WriteLine("  --uninstall        Stop and remove the Windows service (admin; Windows-only).");
        Console.WriteLine("  --register-amsi-provider  Unavailable until secure signed installation exists (exit 7; no changes).");
